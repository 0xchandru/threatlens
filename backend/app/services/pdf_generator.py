...
def generate_ioc_report(scan_data: dict, notes: list, mitre_techniques: list) -> bytes:
    ...
    risk_color = RISK_COLORS.get(risk, COLOR_UNKNOWN)

    confidence = scan_data.get("confidence", "none").upper()
    if confidence == "NONE":
        confidence_label = "NONE — configure API keys for full analysis"
    else:
    ...
    summary_data = [
        ...
        ["Query Time",   f"{scan_data.get('query_time_ms', 0)} ms (6 sources parallel)"],
    ]
    ...
    story.append(Paragraph(
        ...
        "Data sourced from VirusTotal, AbuseIPDB, AlienVault OTX, URLhaus, ThreatFox, MalwareBazaar</font>",
        styles["Normal"]
    ))
